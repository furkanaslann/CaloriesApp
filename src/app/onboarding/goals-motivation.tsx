  ...
  const panResponder = React.useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (_, gestureState) => {
        ...
        const newMotivation = Math.round(
          minMotivation + percentage * motivationRange,
        );
  ...
  return (
    <SafeAreaView style={styles.container} edges={["left", "right", "bottom"]}>
          ...
          <View style={styles.sliderContainer}>
            <View style={styles.sliderBackground}>
              <View style={styles.sliderBar}>
                <View
                  style={[styles.sliderBarFill, { width: sliderPosition }]}
                />
                <View
                  style={[styles.sliderHandle, { left: sliderPosition }]}
                  {...panResponder.panHandlers}
                >
                  <View style={styles.sliderHandleInner}></View>
                </View>
              </View>
            </View>
            <View style={styles.emojiScale}>
              {motivationEmojis.map((emoji, index) => {
                const position =
                  (index / (motivationEmojis.length - 1)) * sliderWidth;
                return (
                  <View
                    key={emoji.value}
                    style={[styles.emojiItem, { left: position }]}
                  >
      ...
      <View style={styles.buttonContainer}>
        <Button title="Devam Et" onPress={handleNext} fullWidth />
      </View>
    </SafeAreaView>
  );
};
