...
export const UserProvider: React.FC<{ children: ReactNode }> = ({
  ...
  useEffect(() => {
    ...
    const initializeAuth = async () => {
      ...
      try {
        ...
        // Continue anyway - firestore will be loaded on demand
      }
  ...
  const loadUserData = (userId: string) => {
    try {
      console.log(
        "loadUserData: Setting up real-time listener for user:",
        userId,
      );
      ...
      const unsubscribe = firestore()
        ...
        .onSnapshot(
          (docSnapshot) => {
            console.log(
              "loadUserData: Snapshot received, exists:",
              docSnapshot.exists,
            );
            ...
            if (docSnapshot.exists()) {
              ...
              if (!rawData || Object.keys(rawData).length === 0) {
                ...
                if (currentUser && !currentUser.isAnonymous) {
                  ...
                  console.log(
                    "🔄 loadUserData: Permanent account with no data - logging out",
                  );
                  auth()
                    .signOut()
                    .catch((logoutError) => {
                      console.error(
                        "❌ loadUserData: Error during logout:",
                        logoutError,
                      );
                    });
                  setUserData(null);
                  ...
                  console.log(
                    "⚠️ loadUserData: Anonymous user with no data - creating minimal structure",
                  );
              ...
              const safeData: UserDocument = {
                ...
                calculatedValues:
                  rawData?.calculatedValues || defaultCalculatedValues,
                progress: rawData?.progress || defaultProgress,
                createdAt:
                  rawData?.createdAt ?? firestore.FieldValue.serverTimestamp(),
                updatedAt:
                  rawData?.updatedAt ?? firestore.FieldValue.serverTimestamp(),
                lastUpdated: rawData?.lastUpdated,
                version: rawData?.version,
              };

              console.log(
                "loadUserData: Safe data created, onboardingCompleted:",
                safeData.onboardingCompleted,
              );
              setUserData(safeData);
            } else {
              console.log(
                "⚠️ loadUserData: No Firestore document found for authenticated user",
              );
              ...
              if (currentUser && !currentUser.isAnonymous) {
                ...
                console.log(
                  "🔄 loadUserData: Permanent account with no document - logging out",
                );
                auth()
                  .signOut()
                  .catch((logoutError) => {
                ...
                // Anonymous user - create minimal data structure for onboarding
                console.log(
                  "⚠️ loadUserData: Anonymous user with no document - creating minimal structure",
                );
                const minimalData: UserDocument = {
                  uid: userId,
                  email: undefined,
  ...
  const createInitialUserDocument = async (userId: string) => {
    try {
      console.log(
        "createInitialUserDocument: Creating document for user:",
        userId,
      );
      ...
      if ((existingDoc as any).exists) {
        console.log(
          "createInitialUserDocument: Document already exists, loading it",
        );
  ...
  const calculateAndUpdateValues = async () => {
    ...
    if (
      ...
      if (profile.gender === "male") {
        ...
        bmr = 447.593 + 9.247 * weight + 3.098 * height - 4.33 * profile.age;
      }
    ...
    if (activity && activity.level) {
      ...
      console.log(
        "calculateAndUpdateValues: No activity level found, using default TDEE",
      );
    }
    ...
    if (goals && goals.primaryGoal) {
      ...
      console.log(
        "calculateAndUpdateValues: No primary goal found, using default TDEE as calorie goal",
      );
    }
    ...
    const hasSignificantChange =
      ...
      Math.abs(
        (currentValues.dailyCalorieGoal || 0) -
          newCalculatedValues.dailyCalorieGoal,
      ) > 50 ||
      Math.abs(
        (currentValues.macros?.protein || 0) -
          newCalculatedValues.macros.protein,
      ) > 5 ||
      Math.abs(
        (currentValues.macros?.carbs || 0) - newCalculatedValues.macros.carbs,
      ) > 5 ||
      Math.abs(
        (currentValues.macros?.fats || 0) - newCalculatedValues.macros.fats,
      ) > 3;

    if (hasSignificantChange) {
      console.log(
        "Calculated values changed significantly, updating Firestore:",
        newCalculatedValues,
      );
      try {
        const db = firestore();
  ...
  const completeOnboarding = async () => {
    ...
    try {
      console.log(
        "completeOnboarding: Starting onboarding completion for user:",
        user.uid,
      );
      ...
      try {
        ...
        console.warn(
          "completeOnboarding: Could not calculate values, but continuing:",
          calcError,
        );
      }
  ...
  const refreshUserData = async () => {
    ...
    console.log(
      "refreshUserData: Data is automatically synced via real-time listener",
    );
  };

  // Sign in with Google
  const signInWithGoogleFn = async (): Promise<FirebaseAuthTypes.User> => {
    try {
      setIsLoading(true);
  ...
  // Sign in with Apple
  const signInWithAppleFn = async (): Promise<FirebaseAuthTypes.User> => {
    try {
      setIsLoading(true);
  ...
  // Sign in with Custom Token (used after OTP verification)
  const signInWithCustomTokenFn = async (
    token: string,
